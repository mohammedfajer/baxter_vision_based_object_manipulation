...
class Tf2Angles:
    ...
    """
        Tf2Angles Transformation (displacement vector) i.e. 3D point of
        a human body joint received from OpenNi NiTE skeleton tracker.

        Compute human joint angles and map them to baxter joint angles.
    """
    ...
    def __compute_joint_angles( self, limb_name ):
        ...
        if success == True:
            ...
            # Compute vectors between frames

            shoulder_2_mirrored_shoulder = self.__math_utils.vector_subtraction_3D( base_2_mirrored_shoulder, base_2_shoulder )
            ...
            # Compute Human Joint Angles

            # Compute S0 Shoulder Roll
            s2e_cross_h2t  = self.__math_utils.cross_product_3D( shoulder_2_elbow, head_2_torso )
            angle_s0 = self.__math_utils.angle_between_two_vectors( shoulder_2_mirrored_shoulder, s2e_cross_h2t )

            if self.__MIRROR:
                if limb_name == 'left':
                    angle_s0 = angle_s0 - math.pi
                else:
                    angle_s0 = -angle_s0 - math.pi

            # Compute S1 Shoulder Pitch
            s2e_cross_h2t_cross_ht2 = self.__math_utils.cross_product_3D( s2e_cross_h2t, head_2_torso )
            angle_s1 = self.__math_utils.angle_between_two_vectors( s2e_cross_h2t_cross_ht2, head_2_torso )

            if self.__MIRROR:
                if limb_name == 'left':
                    angle_s1 = angle_s1 - math.pi
                else:
                    angle_s1 = -angle_s1 - math.pi

            # Compute E0 Elbow Roll
            e2h_cross_s2e = self.__math_utils.cross_product_3D( elbow_2_hand, shoulder_2_elbow )
            angle_e0 = self.__math_utils.angle_between_two_vectors( e2h_cross_s2e, head_2_torso )

            if self.__MIRROR:
                if limb_name == 'left':
                    angle_e0 = angle_e0 - math.pi
                else:
                    angle_e0 = -angle_e0 - math.pi

            # Compute E1 Elbow Pitch
            angle_e1 = self.__math_utils.angle_between_two_vectors( shoulder_2_elbow, elbow_2_hand )
